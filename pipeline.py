...
from conv_3d_model import create_conv_3d
    ...
    def pipeline_step(self, step):
        ...
        if step == 'load from db':
            if self.config.db_type == 'mongo':
                ...
                assert self.data
                model_data = ModelData(self.data, image_folder=self.config.image_folder)
        ...
        if step == 'train':
            ...
            if self.config.model_type == 'Resnet':
                model = create_conv_3d(len(self.config.labels))
            assert model
            evaluator = None
